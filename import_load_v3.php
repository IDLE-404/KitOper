...
/**
 * ИМПОРТ НАГРУЗКИ ИЗ EXCEL - ВЕРСИЯ 3 (поиск по индексу РО/ОН/ПМ)
 * Использование: php import_load_v3.php
 */
...
echo "=== ИМПОРТ НАГРУЗКИ ИЗ EXCEL (ВЕРСИЯ 3) ===\n\n";
...
// Загружаем предметы - по индексу (первое совпадение)
echo "✓ Загрузка предметов...\n";
...
foreach ($subjects2 as $s) {
    $index = extractIndex($s->subject_name);
    if ($index) {
        $key = '2_' . $index;
        if (!isset($subjectsCache[$key])) {
...
// Месяцы для импорта - ТОЛЬКО ФЕВРАЛЬ
$monthsToImport = [2];
$year = 2026;
...
for ($row = 4; $row <= $highestRow; $row++) {
    ...
    // БЕРЁМ ЧАСЫ ИЗ СТОЛБЦА Q (всего часов за 2 семестр)
    ...
    // Ищем предмет по индексу
    $subjectId = findSubjectByIndex($resultIndex, $course, $subjectsCache);
    if (!$subjectId) {
...
function findSubjectByIndex($index, $course, $cache)
{
